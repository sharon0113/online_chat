...
$(function() {
	$.fn.smohanfacebox = function(options) {
		var defaults = {
		Event : "click", //响应事件		
		divid : "{{clientname}}-contact-face", //表单ID（textarea外层ID）
		textid : "{{clientname}}-msg" //文本框ID
		};
		...
		$("#"+options.divid).prepend("<div id='SmohanFaceBox'><span class='Corner'></span><div class='Content'><h3><span>常用表情</span><a class='close' title='关闭'></a></h3><ul>"+faceimg+"</ul></div></div>");
	     $('#SmohanFaceBox').css("display",'none');//创建完成后先将其隐藏//创建表情框结束
		var $facepic = $("#SmohanFaceBox li img");
		//BTN触发事件，显示或隐藏表情层
		$(document).on(options.Event,"#trigger",function(e) {
		...
		$facepic.click(function(){
			...
			//alert($(this).attr("face"));
			 ...
			 //$("#"+options.textid).append($(this).attr("face")) ;
			 //$("#"+options.textid).text( $("#"+options.textid).val() + $(this).attr("face"));
			 $btn.removeClass('in');
			});		
		//关闭表情层
